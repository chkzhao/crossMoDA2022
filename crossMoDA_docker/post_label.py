...
def get_largest_two_component(img, print_info = True, threshold = None):
    """
...
list_case = [k[9:21] for k in os.listdir(image_path)]

for i in list_case:
    print(i)
    label_nib = nib.load(
        os.path.join(image_path, 'crossmoda' + str(i) + '_Label.nii.gz'))
    ...
    label_1 = scipy.ndimage.binary_closing(label_1, structure=struct)
    ...
    label_2 = scipy.ndimage.binary_closing(label_2, structure=struct).astype(np.int16)
    ...
    nib.save(new_label, os.path.join(save_path, 'crossmoda' + str(i) + '_Label.nii.gz'))
