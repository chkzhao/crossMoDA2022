...
for i in list_case:
    print(i)
    label_nib = nib.load(os.path.join(input_dir,'crossmoda'+ i+'_Label', 'crossmoda'+ i+'_Label.nii.gz'))
    img_name = 'crossmoda'+i+'_hrT2.nii.gz'
    img = nib.load(
        os.path.join('./resample', img_name))
    header_img = img.header
    label = label_nib.get_fdata()
    ref_img = nib.load(os.path.join('../../../dataset/validation/validation', img_name))
    ...
    label_name = 'crossmoda'+ i+'_Label.nii.gz'
