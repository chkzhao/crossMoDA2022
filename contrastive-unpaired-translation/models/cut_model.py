...
class CUTModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # # self.netG = nn.SyncBatchNorm.convert_sync_batchnorm(self.netG, process_group)
        self.netG = DDP(self.netG, broadcast_buffers=False)


        if self.isTrain:
            self.netD = networks.define_D(opt.output_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.normD, opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
            # process_group = torch.distributed.new_group()
            # # self.netD = nn.SyncBatchNorm.convert_sync_batchnorm(self.netD, process_group)
            self.netD = DDP(self.netD, broadcast_buffers=False)
    ...
    def data_dependent_initialize(self, data):
        ...
        if self.opt.isTrain:
            ...
            if self.opt.lambda_NCE > 0.0:
                ...
                self.netF = DDP(self.netF, broadcast_buffers=False)
