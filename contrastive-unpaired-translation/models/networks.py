...
from .SN import SpectralNorm, ccbn
...
class AdaptiveInstanceNorm2d(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1,num_classes=2 ):
    ...
    def forward(self, x, y):
        ...
        gamma_reshaped = gamma.contiguous().view(-1)
        beta_reshaped = beta.contiguous().view(-1)
...
def define_G(input_nc, output_nc, ngf, netG='stylegan2', norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], opt=None,minimum=False ,SN=False, multi=False):
    """Create a generator
    ...
    if netG == 'resnet_9blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt, minimum=minimum, SN=SN, multi=multi)
        ...
        net = StyleGAN2Generator(input_nc, output_nc, ngf, use_dropout=use_dropout, opt=opt, minimum=minimum)
...
def define_D(input_nc, ndf, netD='patch', n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None ,minimum=False, SN=False):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        net = NLayerDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, no_antialias=no_antialias, minimum=minimum, SN=SN)
    elif netD == 'patch':
        net= PatchDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, no_antialias=no_antialias)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None, minimum=False, SN=False, multi=False):
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            model_down += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias,SN=SN)]

        model_up = []
        self.bn = ccbn(ngf*mult) if minimum else nn.Identity()
        ...
        self.l_down = len(model_down)
        self.l_up = len(model_up)

        if SN:

            for i in range(self.l_down):
                if isinstance(model_down[i], nn.Conv2d) or isinstance(model_down[i], nn.ConvTranspose2d):
                    model_down[i] = SpectralNorm(model_down[i])

            for i in range(self.l_up):
                if isinstance(model_up[i], nn.Conv2d) or isinstance(model_up[i], nn.ConvTranspose2d):
                    model_up[i] = SpectralNorm(model_up[i])
        if self.minimum:
            self.model_down = nn.Sequential(*model_down)
            self.model_up = nn.Sequential(*model_up)
            self.model = nn.Sequential(*(model_down+model_up))
        else:
            self.model = nn.Sequential(*(model_down+model_up))

    def forward(self, input, layers=[], encode_only=False, d=None, rec=None, alpha=None):
        if -1 in layers:
        ...
        if len(layers) > 0:
            ...
            if self.minimum:
                for layer_id, layer in enumerate(self.model):
                    ...
                    if layer_id == self.l_down:
                        feat = self.bn(feat, d, rec)
                    feat = layer(feat)
            ...
            """Standard forward"""
            if self.minimum:
                coding = self.model_down(input)
                out = self.bn(coding, d, alpha)
                fake = self.model_up(out)
            else:
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias, SN=False):
        ...
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias, SN=SN)

    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias, SN=False):
        ...
        if SN:
            conv_block += [SpectralNorm(nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias)), norm_layer(dim), nn.ReLU(True)]
        else:
        ...
        if SN:
            conv_block += [SpectralNorm(nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias)), norm_layer(dim)]
        else:
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False, minimum=False, SN=False):
        ...
        if self.minimum:
            sequence += [
                nn.Conv2d(ndf * nf_mult, 4, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
        ...
        if SN:
            ...
            for i in range(l):
                if isinstance(self.model[i], nn.Conv2d) or isinstance(self.model[i], nn.ConvTranspose2d):
                    self.model[i] = SpectralNorm(self.model[i])

    def forward(self, input):
        """Standard forward."""
        out = self.model(input)
        if self.minimum:
            h = self.linear(out[:,1:,...].mean(dim=(2, 3)))
            return out[:,0,...], h
        else:
            return out
