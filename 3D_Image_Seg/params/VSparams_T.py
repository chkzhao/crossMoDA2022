...
from utils.utils import (
    ClipIntensityRangePercentilesd, 
    RandBrightnessd, 
    Transformlabeld, 
    RandConvNoised, 
    CutOut, 
    RandBezierTransformd, 
    InstanceNormalizeIntensityd
    )
...
def sigmoid_decay(current_epoch, sum_epoch):
    if sum_epoch == 0:
        return 0.0
    else:
        current = np.clip(current_epoch, 0.0, sum_epoch)
        phase = current / sum_epoch
        return float(np.exp(-5.0 * phase * phase))

def decay_learning_rate(lr, optimizer, cur_epoch, sum_epoch):
    for g in optimizer.param_groups:
        g['lr'] = lr * sigmoid_decay(cur_epoch, sum_epoch)
    return
...
class VSparams:
    def __init__(self, parser):
        ...
        parser.add_argument('--start_epoch', type=int, default=0, help='world size of distrbuted learning')
        ...
        parser.add_argument("--warm_seg", type=int, default=400, help="learning rate at first epoch")
        parser.add_argument("--warm_transfer", type=int, default=800, help="learning rate at first epoch")
        parser.add_argument(
        ...
        self.results_folder_path = os.path.join("../results", self.dataset, self.model + self.args.zoom_model,
                                            ...
                                            ('no_seg_' if self.args.no_seg else '') + ('seg_only_' if self.args.seg_only else '') +
                                              ...
                                              ('noload_' ) +'DA_INS_all_' + self.master_port + self.results_folder_name)
        ...
        if self.dataset == 'both':
            ...
            train_transforms = Compose(
                [
                    ...
                    SpatialPadd(keys=["image", "label"], spatial_size=self.pad_crop_shape),
                    # RandBezierTransformd(keys=["image"], devisor=self.args.intensity, prob=0.7),
                    # ThresholdIntensityd(keys=["image"], threshold=0, above=True),
                    # ThresholdIntensityd(keys=["image"], threshold=self.args.intensity, above=False),
                    # NormalizeIntensityd(keys=["image"], subtrahend=self.args.intensity / 2,
                    #                     divisor=self.args.intensity / 2),
                    ToTensord(keys=["image", "label"]),
                    # RandConvNoised(keys=["image"]),
                    InstanceNormalizeIntensityd(keys=["image"]),
                    # (NormalizeIntensityd(keys=["image"]) if self.args.seg_only else Identityd(
                    #     keys=["image"])),
                    ...
                    # RandBrightnessd(keys=["image"], contrast=(0.7, 1.4), bright=(-5, 5)),
                    # RandGaussianNoised(keys=["image"], std=0.15, prob=0.3),
                    # RandFlipd(keys=["image", "label"], prob=0.5, spatial_axis=0),
                    # RandZoomd(keys=["image", "label"],prob=0.5,mode='nearest'),
                    # RandRotated(keys=["image", "label"],prob=0.5,mode='nearest',range_x=0.2, range_y=0.2,range_z=0.2),
                    # (RandCropByLabelClassesd(
                    #     keys=["image", "label"],
                    #     label_key="label",
                    #     spatial_size=self.pad_crop_shape,
                    ...
                    # #     keys=["image", "label"], roi_size=[self.img_size, self.img_size, 20] ,max_roi_size= self.pad_crop_shape, random_size=True,random_center=True,
                    # # ),
                    # RandZoomd(keys=["image", "label"], min_zoom=0.9, max_zoom=1.1, mode=['area', 'nearest'], prob=0.5),
                    # RandAffined(keys=["image", "label"], rotate_range=(0.17, 0.17, 0.17), shear_range=(0, 0.1),
                    #     translate_range=0.1,
                    #     mode=['bilinear', 'nearest'],
                    #     prob=0.5, padding_mode="border"),
                    # SpatialPadd(keys=["image", "label"], spatial_size=self.pad_crop_shape),
                    # Rand3DElasticd(keys=["image", "label"], sigma_range=(5, 5), magnitude_range=(15, 15), prob=0.5,
                    #     mode=['bilinear', 'nearest']),
                    # ThresholdIntensityd(keys=["image"], threshold=0, above=True),
                    # ThresholdIntensityd(keys=["image"], threshold=self.args.intensity, above=False),
                    # # RandBezierTransformd(keys=["image"], devisor=self.args.intensity, prob=0.7),
    ...
    def set_and_get_optimizer(self, nets):
        ...
        # optimizer_zoom = Ranger(zoom_model.parameters())
        ...
        for epoch in range(num_epochs):
            ...
            for batch in zip_loader:
                ...
                if self.args.no_seg:
                    ...
                    decay_learning_rate(lr=self.initial_learning_rate, optimizer=optimizer, 
                                        cur_epoch=epoch, sum_epoch=200)
